...
exception SerialError of string;
...
fun car (Cons (x, _)) = x
  | car _ = raise SerialError "car: not a Cons cell";

fun cdr (Cons (_, y)) = y
  | cdr _ = raise SerialError "cdr: not a Cons cell";
            ...
            case readNullTerminated () of
              ...
              | SOME "0" => (* CONS *)
                (case !stack of
                   ...
                   | _ => raise SerialError "Not enough elements on stack for CONS")
              | SOME s =>
                if String.isPrefix "1" s then
                    ...
                    raise SerialError ("Invalid string: " ^ s)
    in
        process ();
        BinIO.closeIn inStream;
        case !stack of
            [result] => result
          | _ => raise SerialError ("Invalid file format: stack should have one element, got " ^
