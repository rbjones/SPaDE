...
I do not expect that in my lifetime we will have access to components of a Cosmic Repository located outside our Solar System, but it is intended that the structure delineated will be suitable for unbounded distribution across the cosmos.

The top level abstract descriptions of the [SPaDE](../docs/tlad001.md#spade) repository must therefore address the way in which [SPaDE](../docs/tlad001.md#spade) envisages the integration of very many diverse local repositories into a single coherent cosmic repository.
...
This integration is primarily a way of partitioning the namespace of the [diasporic repository](../docs/tlad001.md#diasporic) into partitions which correspond to the namespaces of the local repositories.
...
This is primarily achieved by the addition of extra levels to the namespace, which introduced a little difficulty in defining what a name is and in the representation of names in the propositions of the repository.
...
There will then also be one or more representations made available through APIs or protocols.
