...
| `retro/` is bootstrap, not SPaDE. HOL written for SPaDE in markdown is part of SPaDE. A specification opens a theory with `force_delete_theory` and an intelligible name of its own. The first specification is a generic abstract syntax taken from the NTBS packing in `krdd004`, not a survival of `spc001`–`spc005`. Those documents are reference. The deductive system remains, expressed so that it is executable. | [amtd006.md](amtd006.md), [amms001.md](amms001.md) |
