...
\maketitle
%\begin{abstract}
%This is a derivative of spc003, part of a suite of documents giving a formal specification of the HOL logic.
...
Though spc001 gives relational specifications of the basic syntactic constructors and various derivatives, spc003 provides the functional versions as well, so they will be used here.

\subsection{Substitution of Equals}

The elaaborate specification of a powerful substitution rule in spc003 will provide a test bed for the kind of meta-theoretic reasoning which is central to this proposal.

Instead of that rule, a small number of much simpler rules will be proposed here, and then an attempt to prove the doundness of the more complex {\it SUBST} rule will be made.

Note that since these rules are given as total functions, special conventions are needed for the value to be returned if the required inference fails.
This will be that the result will be T, the term or the seequent as appropriate.
