...
\usepackage{url}
\makeindex
\title{Mathematical Case Studies: Tools\thanks{
Added to repo 25 March 2012;
...
\bf Copyright \copyright\ : Lemma 1 Ltd 2012--\number\year \\
Reference: LEMMA1/HOL/WRK083; Current git revision: \VCVersion
...
gen_∀_elim
=TEX
\ is ∀-elimination combined with renaming of type variables to avoid capture.
=SML
...
fun ⦏morphism_conv⦎
	...
	: CONV = (
	let	val unary_thms = map (fn t => all_∀_intro1 pattern_vars t (gen_∀_elim t unary_rule_thm))
			unary;
...
=TEX
.
The tactic begins by η-expanding $f$ if it not already an abstraction.
=SML
...
=TEX
Now the tactic. After the η-expansion and rewriting discussed above,
...
fun ⦏ho_bc_thm_tac⦎ (thm : THM) : TACTIC = ( 
	...
	let	val nbvs = list_variant (frees conc) bvs;
		...
		val thm4 = conv_rule (TRY_C
				(LEFT_C all_simple_β_conv AND_OR_C
					RIGHT_C simple_β_η_norm_conv)) thm3;
		val thm5 = bc_rule thm4;
	in	TRY (conv_tac simple_β_η_norm_conv) THEN ⇒_thm_tac thm5
	end	gl
	end
);
end;
